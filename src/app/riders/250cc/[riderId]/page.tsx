...
export default function RiderDetailPage({ params }: { params: { riderId: string } }) {
  ...
  const [pointsData, setPointsData] = useState<{ supercross: number | undefined; motocross: number | undefined; playoffs: number | undefined; } | null>(null);

  useEffect(() => {
    if (rider) {
      const points = getSeriesPoints();
      const riderPoints = {
          supercross: points.supercross250East.find(p => p.rider === rider.name)?.points ?? points.supercross250West.find(p => p.rider === rider.name)?.points,
          motocross: points.motocross250.find(p => p.rider === rider.name)?.points,
          playoffs: points.playoff250.find(p => p.rider === rider.name)?.points,
      };
